...
while ($completed -lt $videoFiles.Count) {
    ...
    $percentage = if ($videoFiles.Count -gt 0) { ($completed / $videoFiles.Count) * 100 } else { 0 }
    Write-ProgressBar -Percentage $percentage -Line 2
    $w = $Host.UI.RawUI.WindowSize.Width
    $i=0; $recent|%{[Console]::SetCursorPosition(0,4+$i++);$s=if($_.IsCorrupted){"[X]"}else{"[V]"};Write-Host "$s $(Truncate-Path $_.FullName ($w-5))" -NoNewline;[Console]::Write(" "*($w-($Host.UI.RawUI.CursorPosition.X)))}
    Start-Sleep -ms 100
}
    ...
    if (Test-UserConfirmation -Prompt $confirmMsg) {
        ...
        $corruptedFiles |% {
            ...
            try {
                ...
                $status = "failed to " + $action.ToLower()
                Write-Log ($lang.action_failed -f $file.FullName, $_.Exception.Message, ($config.language -eq 'fr' ? $action.ToLower()+'r' : $action.ToLower()))
                $reports+=[PSCustomObject]@{Type="Corrupted";Status=$status;Details=$_.Exception.Message;Path=$file.FullName}
            }
        }
    } else {
        $actionCancelled = $config.language -eq 'fr' ? $actionFr : $action
        Write-Host ($lang.action_cancelled -f $actionCancelled, $actionPast) -FG Yellow
